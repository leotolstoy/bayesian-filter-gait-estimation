...
%         incline_scaled = (treadmillIncline - inclineMin)/(inclineMax - inclineMin);
        ...
        [rows,cols] = size(A_dataport_mat);
        subjMatSize = subjMatSize + rows;
...
%         concatenated_data(:, numel(trials)*(s-1)+t ) = nanmean(temp_data,2);
    ...
    subjIdxs = [subjIdxs;[1 + idxOffset, subjMatSize + idxOffset]]
    idxOffset = idxOffset + subjMatSize
...
[rows,cols] = size(A_dataport_mat_master)
...
for sub_to_exclude_idx = 1:length(sub)
    filename = sprintf('gaitModel_CrossVal_exclude%s.csv',sub{sub_to_exclude_idx})
    excludeIdxs = subjIdxs(sub_to_exclude_idx,1):subjIdxs(sub_to_exclude_idx,2);
    includeIdxs = setdiff(1:rows, excludeIdxs);
    
    A_dataport_mat_excluded = A_dataport_mat_master(includeIdxs,:);
    b_dataport_footAngle_excluded = b_dataport_footAngle_master(includeIdxs,:);
    b_dataport_shankAngle_excluded = b_dataport_shankAngle_master(includeIdxs,:);
    b_dataport_thighAngle_excluded = b_dataport_thighAngle_master(includeIdxs,:);
    b_dataport_pelvisAngle_excluded = b_dataport_pelvisAngle_master(includeIdxs,:);
    
    best_fit_params_footAngle = lsqlin(A_dataport_mat_excluded,b_dataport_footAngle_excluded,[],[],A_eq_foot, b_eq_foot);
    best_fit_params_shankAngle = lsqlin(A_dataport_mat_excluded,b_dataport_shankAngle_excluded,[],[],A_eq_shank, b_eq_shank);
    best_fit_params_thighAngle = lsqlin(A_dataport_mat_excluded,b_dataport_thighAngle_excluded,[],[],A_eq_thigh, b_eq_thigh);
    best_fit_params_pelvisAngle = lsqlin(A_dataport_mat_excluded,b_dataport_pelvisAngle_excluded,[],[],A_eq_pelvis, b_eq_pelvis);
